...
const banks = ["ABN AMRO", "ASN Bank", "bunq", "ING", "Rabobank", "RegioBank", "SNS"];
...
const productOptions = {
  inboedel: {
    ...
    checks: [
      ...
      "Schade door een ongelukje",
      "Schade ontstaan door cybercriminaliteit"
    ]
  },
  opstal: {
    label: "Opstal",
    price: 12.75,
    checks: [
      "Schade door brand, neerslag, storm",
      "Kosten voor vervangende woonruimte bij schade",
  ...
  app.innerHTML = `
    <div class="page">
      <div class="wrap">
        <header class="top">
          <div>
            <p class="eyebrow">Prototype verzekering aanvragen</p>
            <h1 class="title">Woonverzekering</h1>
          </div>
...
function renderPremie() {
  ...
  shell(stepCard("Premie berekenen", "Stap 2", `<div><span class="field"><span>Kies je verzekering</span></span><div class="product-grid">${products}</div></div><div class="notice"><b>Indicatieve premie:</b> ${euro(premium())} per maand</div>${idinNote()}`), { back: "back" });
}

function renderAanvullend() {
  shell(stepCard("Aanvullende gegevens", "Stap 3", `${yesNo("inwonend", "Woont er iemand in de woning?")}${field("ingangsdatum", "Gewenste ingangsdatum", "", "date")}`), { back: "back" });
}

function renderOverzicht() {
  const chosen = state.form.products.map(p => productOptions[p].label).join(", ") || "Nog niet gekozen";
  const body = `<div class="summary"><b>Controleer je gegevens</b><br>Adres: ${escapeHtml(state.form.postcode)} ${escapeHtml(state.form.huisnummer)}<br>Verzekering: ${chosen}<br>Ingangsdatum: ${escapeHtml(state.form.ingangsdatum)}<br>Premie: ${euro(premium())} per maand</div>
...
function renderIdin() {
  ...
  if (state.idinPhase === "intro") {
    ...
    const rabobankLogo = state.selectedBank === "Rabobank" ? `<img src="assets/rabobank-logo.png" alt="Rabobank logo" onerror="this.style.display='none'">` : "";
    html = `<div class="idin-screen"><div class="bank-app-wide"><div class="bank-app-header"><span>☰</span><b>${escapeHtml(state.selectedBank)} app</b></div><div class="bank-app-body"><div class="bank-app-brand">${rabobankLogo}<p class="muted"><b>${escapeHtml(state.selectedBank)}</b> app</p></div><p class="section-label" style="color:#047857">Identiteitsverzoek</p><h2>Bevestig je gegevens</h2><p class="muted" style="line-height:1.7">Verzekeraar.nl vraagt om je identiteit te bevestigen via iDIN.</p><div class="identity-grid"><span>Naam</span><b>Sam de Vries</b><span>Geboortedatum</span><b>14-05-1988</b><span>Adres</span><b>Voorbeeldstraat 12</b></div><div class="notice" style="margin-top:18px">Controleer de gegevens en bevestig de aanvraag in je bank app.</div><button class="btn green" data-action="idin-done" style="width:100%;margin-top:22px">Bevestigen</button><button class="btn outline" data-action="back-to-overzicht" style="width:100%;margin-top:12px">Annuleren</button></div></div></div>`;
  }
  shell(html, { external: true, back: "back-to-overzicht" });
}

function renderGegevensAanvullen() {
  const html = `<div class="grid"><div class="content"><p class="section-label">Stap 5</p><h2>Gegevens aanvullen</h2><div class="idin-success"><b>iDIN succesvol</b><br>Naam, geboortedatum en adres zijn opgehaald via ${escapeHtml(state.selectedBank)}.</div><div class="stack" style="margin-top:18px"><div class="form-row">${field("email", "E-mailadres", "naam@email.nl")}${field("telefoon", "Telefoonnummer", "06 12345678")}</div>${field("rekening", "Rekeningnummer", "NL00 BANK 0123 4567 89")}<div class="question"><p>Marketing opt-ins</p><label class="optin"><input data-field="optinEmail" type="checkbox" ${state.form.optinEmail ? "checked" : ""}> E-mail</label><label class="optin"><input data-field="optinSms" type="checkbox" ${state.form.optinSms ? "checked" : ""}> SMS</label><label class="optin"><input data-field="optinTelefoon" type="checkbox" ${state.form.optinTelefoon ? "checked" : ""}> Telefoon</label></div></div><button class="btn" data-action="finish" style="margin-top:26px">Verzekering afsluiten</button></div>${benefits()}</div>`;
  shell(html, { back: "back-to-overzicht" });
}
...
function render() {
  ...
  else if (state.step === 4) renderGegevensAanvullen();
  else renderThanks();
}
...
function bindEvents() {
  document.querySelectorAll("[data-action]").forEach(el => el.addEventListener("click", e => {
    ...
    if (action === "back-to-overzicht") setState({ step: 3, idinPhase: "intro", selectedBank: "" });
    ...
    if (action === "restart") {
      state.step = 0;
      state.idinPhase = "intro";
      state.selectedBank = "";
      state.showIdinInfo = false;
      render();
    }
  }));
